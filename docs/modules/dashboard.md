...
None of these endpoints are currently called by the dashboard — wiring is pending (gap **G-DASH-1**, see [IMPLEMENTATION_AUDIT.md](../implementation-audit.md)).
...
- **API contract, stable today:** `/api/work-orders/kpi/summary`, `/api/quotes/kpi/open-count`, `/api/costs/kpi/material-transit` — assert response shape against a seeded DB (see [REGRESSION_REQUIREMENTS.md](../regression-requirements.md) §Dashboard).
