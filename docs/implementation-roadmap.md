...
**Config migration — hardcoded → system_config (2026-04-23 · Phase-5b PR-2).** Los valores sembrados en Phase-5b ahora se leen en caliente desde `system_config`. Cambios aplicados: (a) `forecasting.js` lee `forecasting.semaforo.ok_max` / `atencion_max` / `system.exchange_rate_fallback` en cada request (classify recibe `thresholds` inyectadas); (b) `approvals.js` toma `approval.steps` + `approval.roles.<step>` dinámicamente — la sub-tabla de rows nuevos, el orden secuencial y el rol-gate leen de config en cada transición; (c) nueva middleware `verificarRolDinamico(configKey, ...fallbackRoles)` gatea `/conciliacion/forzar` con `conciliacion.forzar.roles` y `/conciliacion/justificar` con `conciliacion.justificar.roles`; el service `forzarConciliacion` hace defense-in-depth leyendo la misma key; (d) `POST /api/work-orders` auto-rellena `jefeIngenieria/Manufactura/Compras/Otros` desde `ot.jefaturas_default` (operator-provided wins, defaults fill blanks); (e) `POST /api/employees` usa `conciliacion.turno_default`. 14 claves semilla en total (agregada `conciliacion.justificar.roles`). Live-verified: cambiar un threshold en el panel admin flip el semáforo en el siguiente request sin reiniciar.

**Phase-4 skeleton — Nómina (2026-04-23 · P4.2 · G-NOM-4).** Backend completo listo para cuando el cliente firme la plantilla en P4.1. Nueva migración `20260423-0006-nomina-skeleton.js` con dos tablas: `payroll_weeks(anio/mes/bimestre/semana, fecha_inicio/fin, cerrada, cerrada_por/at, notas)` UNIQUE(anio, semana), y `payroll_lines` con snapshot denormalizado del empleado (numero_lista/nombre/rfc/curp/imss/puesto/area/turno/fecha_ingreso/sd/sdi) + totales tipados (total_percepciones, total_deducciones, percepciones_gravadas, pago_nomina, dias_nomina, hrs_laboradas) para agregaciones SUM sin parsear JSON, + `detalle JSONB` para la granularidad de las ~70 columnas adicionales cuyo layout exacto depende de P4.1. UNIQUE(week, empleado). Modelos + asociaciones a `Employee`. Nuevo router `routes/payroll.js` con CRUD de weeks + lines, KPI `/resumen` con SUM agregados, y `/export` XLSX de 14 cols. Seed: una semana demo (2026 · semana 16, 13-17 Abr). Hooks de auditoría activos. FE: selector de semana en el sub-tab Captura, modal "+ Nueva semana", Resumen KPIs en vivo desde `/resumen`, botón "+ Nueva línea" deshabilitado con tooltip explicando el bloqueo P4.1. **G-NOM-3 cerrado** — handler dedicado `handleNominaCfdiUpload` reemplaza el misrouting hacia Facturas (el parser real del complemento 1.2 llega en P4.11). Las calculadoras IMSS/ISR/INFONAVIT/FONACOT NO están — requieren las tablas fiscales 2026 + 20 escenarios de referencia a validar con cliente.
