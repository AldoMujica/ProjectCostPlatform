...
> **Feeder documents:** [`IMPLEMENTATION_AUDIT.md`](./IMPLEMENTATION_AUDIT.md) (gap catalogue), [`REGRESSION_REQUIREMENTS.md`](./REGRESSION_REQUIREMENTS.md) (211+ ACs), [`designs/00-architecture-decisions.md`](./designs/00-architecture-decisions.md) (ADR), [`project-summary.md`](./project-summary.md) (module map).
>
...
| ID    | Item                                                             | Owner | Days | ADR | Gap closed  |
|-------|------------------------------------------------------------------|-------|-----:|-----|-------------|
...
| P1.7  | Per-record access filter extended from `filtrarPorSupervisor`    | BE    | 1    | 005 | (new)       |
| P1.8  | Migration: extend `WorkOrder` w/ liberation-form fields          | BE    | 2    | —   | G-OT-2      |
...
| P1.12 | **FK migration:** add `work_order_id` to child tables, backfill, lock | BE | 2    | 006 | D-fk         |
| P1.13 | Replace `Supplier.workOrders[]` w/ `supplier_work_orders` join    | BE    | 1    | 006 | D-fk         |
| P1.14 | Migration tooling: replace `sequelize.sync({alter:true})` w/ umzug | BE  | 1    | 003 | (new)       |
