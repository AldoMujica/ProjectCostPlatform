...
> **Audit date:** 2026-04-20 (against `alenstec_app.html` at commit `3c22899` baseline + uncommitted backend state).
>
...
>
> **Headline:** Of the ~115 distinct features identified in the mockup, **~20 %** are fully wired end-to-end. The rest is either static HTML (mockup) or a half-stack (model exists but no UI wiring, or UI exists but no backend).
...
|----------------------|:------:|---------------------------------------------|------------|
| 4-KPI strip          | ⚠️      | Endpoints exist, UI not wired               | G-DASH-1   |
...
| Production JWT enforcement           | ⚠️      | Dev fallback leaks admin role                       | G-CONC-6  |
| Default JWT secret                   | ⚠️      | `'tu_secret_key'` if env unset                      | G-CONC-7  |
| `asistencia-modulo/` vs backend/src  | ⚠️      | Two parallel implementations                        | G-CONC-8  |
...
- **G-EXP-1** — Wire 11 XLSX no-op buttons. Single architecture decision (client vs. server XLSX) unlocks 11 tables.
- **G-OT-2** — Extend `WorkOrder` schema to cover liberation-form fields. Blocks OT save, OT approval workflow, Pronóstico aggregation.
- **G-CONC-6 / G-CONC-7** — Replace JWT dev fallback and default secret before production.
...
- **G-CONC-8** — Decide: backend/src or asistencia-modulo as canonical. Remove the other.
- **G-MAT-4, G-COT-5, G-PROV-4** — Model-field audits and migrations.
- **html2canvas removal or feature addition.**
- Remove / update legacy docs that don't match HTML: `CONCILIACION_README.md` references migration files under `backend/src/db/migrations/` that do not exist in the backend tree (they live in `asistencia-modulo/` instead) — reconcile.

---

## Risk summary

1. **Dual-server architecture (`:3000` legacy + `:3001` conciliación)** is unconventional and likely to cause deployment confusion. Merge into a single Express app in Phase 2.
2. **No foreign-key constraints** in the Sequelize models — everything references `otNumber` as a free string. Rename collisions or deletes will silently orphan data. Address before going live.
3. **Two parallel conciliación implementations** (`backend/src/` + `asistencia-modulo/`) — a maintenance bomb. Pick one.
4. **No authentication** on the cost API (`/api/work-orders`, `/quotes`, `/costs`, `/suppliers`). Safe only while the backend is inaccessible.
5. **Default JWT secret** in development mode leaks admin role silently.
6. **Static mockup data baked into HTML** confuses testers — a row in the Recent-OT table might look "wired" but is just HTML.
